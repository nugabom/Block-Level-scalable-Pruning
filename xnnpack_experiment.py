...
if __name__ == "__main__":
    ...
    print(f"get sparse model for {density}")
    ...
    if os.path.exists(os.path.join(FLAGS.log_dir, 'latest_checkpoint.pt')):
        ...
        for key_old in old_keys:
            ...
            if 'module' in key:
                key = key_old[7:]
            new_checkpoint[key] = checkpoint[key_old]
    ...
    layer_idx = 0
    layer_index = []
    prunable_layer_index = []
    for key, value in debug.items():
        print(f"{key} = {value}")
    for conv in model.modules():
        if isinstance(conv, nn.Conv2d):
            # weight
            layer_index.append(f"w{layer_idx+65}")   
            # bias
            layer_index.append(f"w{layer_idx+65+1}")
            if isinstance(conv, DynamicGroupConv2d):
                prunable_layer_index.append(f"w{layer_idx+65}")
            layer_idx += 2
    
    prunable_layer = [layer.weight.cpu().view(-1).detach().numpy().tolist() for layer in model.modules() if isinstance(layer, DynamicGroupConv2d)]

    folder_name = f"xnnpack_{FLAGS.pruner}__mobilenetV2"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    with open(os.path.join(folder_name, f"{density}_fp32-sparse-mobilenet-v2.cc"), 'w') as f:
        f.write(top)

        for arr, value_list in zip(prunable_layer_index, prunable_layer):
            text = f"\t{arr} = " + "{\n"
